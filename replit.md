...
- **AI Agent Core**: Modular agent architecture located in `server/agent/`, handling prompt generation, tool definitions, execution, retry mechanisms, and validation. The agent supports a tool confirmation pattern for immediate action. Model references are centralised via `AGENT_MODEL` / `AGENT_MODEL_LIGHT` / `AGENT_MODEL_DEEP` constants in `server/agent/executor.ts` (currently `gpt-4.1` / `gpt-4.1-mini` / `gpt-5`). The App Brain document (`server/agent/appKnowledge.ts`) provides domain routing knowledge across all 12 data domains and is injected into every system prompt. A user profile (`server/agent/userProfile.ts`) is injected into the system prompt before the App Brain, calibrating agent tone for a senior FI banking executive. The "Deep Think" mode allows users to opt-in to `gpt-5` for individual messages via a toggle in the chat UI header — the flag flows from frontend → `/api/chat` body → `runChat()` → `runAgentLoop()` model override.
...
- **OpenAI API**: Utilized for AI functionalities including:
    ...
    - GPT-5 (`AGENT_MODEL_DEEP`) for optional "Deep Think" mode — user-activated via toggle in chat UI header.
