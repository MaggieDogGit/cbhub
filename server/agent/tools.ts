...
export function getFmiTaxonomyTools(): any[] {
  return [
    ...
    { type: "function", function: { name: "create_fmi_entry", description: "Create a new FMI Taxonomy catalogue entry. Use this to add a new payment system, clearing mechanism, settlement system, or other market infrastructure. Always call find_fmi_entries first to confirm the entry does not already exist. Requires category_id — call list_fmi_categories first to find the correct UUID.", parameters: { type: "object", required: ["name", "code", "category_id"], properties: { name: { type: "string", description: "Full official name, e.g. 'Lynx', 'BOJ-NET Funds Transfer System'" }, short_name: { type: "string", description: "Abbreviated name, e.g. 'Lynx', 'BOJ-NET'" }, code: { type: "string", description: "Unique short code, e.g. 'LYNX', 'BOJNET', 'RIX'" }, category_id: { type: "string", description: "UUID of the FMI category — use list_fmi_categories to find it" }, status: { type: "string", enum: ["live", "planned", "decommissioned"], description: "Defaults to 'live'" }, description: { type: "string" }, operator_name: { type: "string", description: "Name of the operating institution, e.g. 'Payments Canada', 'Bank of Japan'" }, settlement_model: { type: "string", description: "e.g. 'RTGS', 'DNS', 'Hybrid'" }, supports_24x7: { type: "boolean" }, supports_cross_border: { type: "boolean" }, supports_one_leg_out: { type: "boolean" }, primary_currency_code: { type: "string", description: "ISO 4217 currency code, e.g. 'CAD', 'JPY', 'SEK'" }, notes: { type: "string" } } } } },
    { type: "function", function: { name: "update_fmi_entry", description: "Update an existing FMI Taxonomy catalogue entry by ID. Can modify fields like supports_one_leg_out, supports_cross_border, supports_24x7, settlement_model, status, description, notes, operator_name, primary_currency_code.", parameters: { type: "object", required: ["id"], properties: { id: { type: "string" }, name: { type: "string" }, description: { type: "string" }, status: { type: "string" }, settlement_model: { type: "string" }, supports_24x7: { type: "boolean" }, supports_cross_border: { type: "boolean" }, supports_one_leg_out: { type: "boolean" }, primary_currency_code: { type: "string" }, operator_name: { type: "string" }, notes: { type: "string" } } } } },
