...
export function getCbTaxonomyTools(): any[] {
  return [
    ...
    { type: "function", function: { name: "update_cb_capability_value", description: "Upsert a capability score for a banking group against a taxonomy item. Sets the value for the given banking_group_id + taxonomy_item_id combination.", parameters: { type: "object", required: ["banking_group_id", "taxonomy_item_id", "value"], properties: { banking_group_id: { type: "string" }, taxonomy_item_id: { type: "string" }, value: { type: "string", description: "The capability value (format depends on taxonomy item's value_type)" }, legal_entity_id: { type: "string" }, correspondent_service_id: { type: "string" } } } } },
  ];
}
